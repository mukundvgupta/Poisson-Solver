      ...
      REAL, ALLOCATABLE :: d1(:),d2(:),d3(:),X(:),Y(:),Z(:),b(:)
      REAL, ALLOCATABLE :: S(:,:,:),P(:,:,:)
      PI = ATAN(1.)*4.
      ...
      ALLOCATE (d1(N-1),d2(N-1),d3(N-1),X(N+1),Y(N+1),Z(N+1),b(N-1))
...
c--sweep in z direction
      DO  I = 2,N
         DO J = 2,N
            DO L = 2,N
               b(L-1)=S(I,J,L)-(P(I+1,J,L)+P(I-1,J,L))/DS2
               b(L-1)=b(L-1) - (P(I,J+1,L)+P(I,J-1,L))/DS2
            END DO
...
c--sweep in x direction
      DO  J = 2,N
         DO L = 2,N
            DO I = 2,N
               b(I-1) = S(I,J,L)-(P(I,J,L+1)+P(I,J,L-1))/DS2
               b(I-1) = b(I-1) - (P(I,J+1,L)+P(I,J-1,L))/DS2
            END DO
            CALL THOMAS(d1,d2,d3,b,N-1)
            P(2:N,J,L) = b(1:N-1)
         END DO
...
c--sweep in y direction
      DO  I = 2,N
         DO L = 2,N
            DO J = 2,N
               b(J-1) = S(I,J,L)-(P(I+1,J,L)+P(I-1,J,L))/DS2
               b(J-1) = b(J-1) - (P(I,J,L+1)+P(I,J,L-1))/DS2
            END DO
            CALL THOMAS(d1,d2,d3,b,N-1)
            P(I,2:N,L) = b(1:N-1)
         END DO
...
c--compute the residual
      ...
      DO  I = 2,N
         DO L = 2,N
            DO J = 2,N
               RES = S(I,J,L)-(P(I+1,J,L)-2.*P(I,J,L)+P(I-1,J,L))/DS2
               RES = RES     -(P(I,J+1,L)-2.*P(I,J,L)+P(I,J-1,L))/DS2
               RES = RES     -(P(I,J,L+1)-2.*P(I,J,L)+P(I,J,L-1))/DS2
               RES = ABS(RES)
